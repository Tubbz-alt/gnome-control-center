...
ChargeProfile charge_profile_from_thresholds (guchar start, guchar end) {
  ...
  else if (start == 78 && end == 80)
    return CHARGE_PROFILE_BALANCED;
  else if (start == 0 && end == 100)
    return CHARGE_PROFILE_FULL_CAPACITY;
  else
...
void charge_profile_get_thresholds (ChargeProfile profile, guchar *start, guchar *end) {
  switch (profile) {
    ...
    case CHARGE_PROFILE_BALANCED:
      ...
      *end = 80;
      break;
    case CHARGE_PROFILE_FULL_CAPACITY:
      *start = 0;
      *end = 100;
      break;
    default:
...
struct _CcChargeThresholdDialog {
  ...
  GtkListBoxRow *full_capacity_row;
  GtkRadioButton *max_lifespan_radio;
  GtkRadioButton *balanced_radio;
  GtkRadioButton *full_capacity_radio;
...
radio_toggled_cb (CcChargeThresholdDialog *self, GtkRadioButton *radio)
{
  ...
  else if (radio == self->full_capacity_radio)
    profile = CHARGE_PROFILE_FULL_CAPACITY;
  else
    return;
...
row_activated_cb (CcChargeThresholdDialog *self, GtkListBoxRow *row)
{
  ...
  else if (row == self->full_capacity_row)
    gtk_button_clicked (GTK_BUTTON (self->full_capacity_radio));
}
...
cc_charge_threshold_dialog_class_init (CcChargeThresholdDialogClass *klass)
{
  ...
  gtk_widget_class_bind_template_child (widget_class, CcChargeThresholdDialog, full_capacity_row);
  ...
  gtk_widget_class_bind_template_child (widget_class, CcChargeThresholdDialog, full_capacity_radio);
...
cc_charge_threshold_dialog_new (S76PowerDaemon *power_proxy, ChargeProfile profile)
{
  ...
  switch (profile) {
    ...
    case CHARGE_PROFILE_BALANCED:
      ...
      break;
    case CHARGE_PROFILE_FULL_CAPACITY:
      radio = dialog->full_capacity_radio;
      break;
    default:
      break;
  }
