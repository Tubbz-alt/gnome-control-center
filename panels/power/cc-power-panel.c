...
charge_thresholds_ready(GObject *source_object,
  ...
  profile = charge_profile_from_thresholds (start, end);
  self->charge_profile = profile;
  gtk_label_set_label (GTK_LABEL (self->threshold_label), charge_profile_get_title (profile));
}
...
add_battery_section (CcPowerPanel *self)
{
  ...
  hdy_preferences_row_set_title (HDY_PREFERENCES_ROW (threshold_row), "Thresholds");
